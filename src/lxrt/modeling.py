...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, mapper=None, wiki_emb=None, tokenizer=None, surface2wiki = None):
        ...
        debug = False ## <--- SEE TO TRUE FOR DETAILED DEV PURPOSES or FALSE to hide
        ...
        if mapper is not None: 
            ...
            for r in range(input_ids.shape[0]):
                ...
                #if debug:
                #    if surface2wiki != None:
                #        print("surface2wiki[r] for ",r," is ", surface2wiki[r])
                ...
                for i in range(int(len(unk_locs) / 2)):
                   ...
                   if cond1 or cond2 or cond3:
                       ...
                       try:
                           ...
                           if debug:
                               print("Without surface2wiki Checking if ",ent_title," in wiki_emb")
                ...
                if surface2wiki == None: # or ( surface2wiki[r] == [] ):
                    ...
                    if surface2wiki[r] == [] or all_have_no_wik_ents_matched:
                        if debug:
                            print("IN SURFACE2WIKI == [] with input_ids[r]: r", r, input_ids[r], len(input_ids), type(input_ids), input_ids.size())
                        ...
                        if 100 in input_ids[r]:
                            ...
                            if input_ids.size(0) == 1:
                                # for testing, batchsize=1, just use it
                              
                                del input_ids
                                input_ids = torch.tensor(tmp_vec ).unsqueeze(0).cuda()
                            else:
                                #for training, pad tmp_vect so it has correct size of existing tensor
                                #not solved by detaching, changing and putting back on gpu
                                #input_ids = input_ids.detach()
                                #input_ids[r] = torch.nn.functional.pad(tmp_vec, pad=(0,input_ids[r].size(0) - tmp_vec.size(0)))
                                tmp_vec_pad = torch.nn.functional.pad(tmp_vec, pad=(0,input_ids[r].size(0) - tmp_vec.size(0)))
                                cur_embedding = self.word_embeddings(tmp_vec_pad)
                        ...
                        cur_embedding, fin_toks = self.get_enhanced_embedding( input_ids, r, slash_emb, cur_ex, cur_ex_str, cur_ex_toks, cur_ents, unk_locs, tokenizer, debug, debug_end, debug_final)
            ...
            debug_captplain = False
            if debug_captplain:
                toks = []
                for r in range(input_ids.shape[0]):
                    cur_ex = input_ids[r]
                    cur_ex_toks = tokenizer.convert_ids_to_tokens([int(t) for t in cur_ex])
                    toks.append(cur_ex_toks)
                print("Plain/Capt Getting embeddings for: ")
                for ii, ex in enumerate(toks):
                    print(ii,len(ex), ex)
            ...
            if debug:
                print("NORM Dims Words", words_embeddings.shape, "Positions: ", position_embeddings.shape, "TokenTypes: ", token_type_embeddings.shape)
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        #print("LXRT encoder with %d l_layers, %d x_layers, and %d r_layers." % (self.num_l_layers, self.num_x_layers, self.num_r_layers))
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if mapper is None:
            embedding_output = self.embeddings(input_ids, token_type_ids, tokenizer=tokenizer)

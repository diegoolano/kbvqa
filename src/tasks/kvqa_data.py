...
class KVQADataset:
    ...
    """
    def __init__(self, splits: str, incl_para: bool, incl_caption: bool, use_lm: None, ent_set: None, use_capt_sep=False, use_dpr=False, debug=False):
        ...
        if use_dpr:
            if use_dpr == "oracle_dpr":
                dpr_dict = json.load(open(abs_path + "data/kvqa/new_kvqa_q_caps_ents0502_dpr_top5.json"))
            elif use_dpr == "capt_dpr":
                dpr_dict = json.load(open(abs_path + "data/kvqa/kvqa_ent_spans_sept13_fewkbcor_dpr_top5.json"))  #TODO figure out how to handle

        if use_lm == "ebert":
            if ent_set == None:
                entity_spans_dict = json.load(open(abs_path + "data/kvqa/new_kvqa_q_caps_ents0502.json"))
            elif ent_set == "oracle_links":
                entity_spans_dict = json.load(open(abs_path + "data/kvqa/kvqa_oracle_ent_spans_with_wikis0923.json"))
            ...
            if debug:
                ...
                # self.ent_span_len = len(entity_spans_dict[ks[0]]['ents_found'][0])  force size 4
            ...
            for r in cur_split:
                ...
                for i,q in enumerate(r['Questions']):
                    ...
                    if use_lm == "ebert":
                        ...
                        if use_dpr:
                            cur_dpr_spans = dpr_dict[image_id]
                            d['sent'] = " [SEP] ".join([q] + [t["title"] + " " + t["text"] for t in cur_dpr_spans[str(i)]["dpr"][0:3]])      
                        elif use_capt_sep:
                        ...
                        #d['sent'] = r['wikiCap']+". "+q if incl_caption else q    #PRIOR EXPERIMENT
                        if use_dpr:
                            cur_dpr_spans = dpr_dict[image_id]
                            d['sent'] = " [SEP] ".join([q] + [t["title"] + " " + t["text"] for t in cur_dpr_spans[str(i)]["dpr"][0:3]])      
                        elif use_capt_sep:
...
class KVQATorchDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        if self.raw_dataset.use_lm == "ebert":
           ...
           ques = datum['sent']
           ent_spans = datum['ents_found']     # this is a list that can be of size 20 max 
           #print(ent_spans, len(ent_spans), len(ent_spans[0]))
        ...
        if 'label' in datum:
            ...
            #print("LOOKING AT ENTSPANS", len(ent_spans), [ len(e) for e in ent_spans ])

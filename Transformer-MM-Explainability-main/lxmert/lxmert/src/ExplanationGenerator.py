...
def apply_self_attention_rules(R_ss, R_sq, cam_ss):
    #print("apply_self_attention_rules", R_ss.size(), R_sq.size(), cam_ss.size())
    # apply_self_attention_rules torch.Size([37, 37]) torch.Size([37, 36]) torch.Size([44, 44])
    R_sq_addition = torch.matmul(cam_ss, R_sq) # RunError: mat1 dim 1 must match mat2 dim 0
...
class GeneratorOurs:
    ...
    def generate_ours(self, input, index=None, use_lrp=True, normalize_self_attention=True, apply_self_in_rule_10=True, method_name="ours", model=None, output=None):

        #print("In GENERATE_OURS with")
        #print("input",input)
        ...
        #print("In generate ours with text_tokens", text_tokens, "and image bboxes", image_bboxes, "output.size()", output.size(), "and qa score:", output)

        """
        Traceback (most recent call last):
          File "lxmert/lxmert/perturbation_okvqa.py", line 309, in <module>
            main(args)
            ..
          ...
          File "/home/diego/adv_comp_viz21/lxmert/orig_code/lxmert_gen/Transformer-MM-Explainability-main/lxmert/lxmert/src/ExplanationGenerator.py", line 177, in generate_ours
            self.handle_self_attention_lang(blocks)

          File "/home/diego/adv_comp_viz21/lxmert/orig_code/lxmert_gen/Transformer-MM-Explainability-main/lxmert/lxmert/src/ExplanationGenerator.py", line 69, in handle_self_attention_lang
            R_t_t_add, R_t_i_add = apply_self_attention_rules(self.R_t_t, self.R_t_i, cam)

          File "/home/diego/adv_comp_viz21/lxmert/orig_code/lxmert_gen/Transformer-MM-Explainability-main/lxmert/lxmert/src/ExplanationGenerator.py", line 27, in apply_self_attention_rules
        ...
        self.R_t_t = torch.eye(text_tokens, text_tokens).to(model.device)   #20x20
...
class GeneratorBaselines:
    ...
    def generate_transformer_attr(self, input, index=None, method_name="transformer_attr", model = None, output = None):
        kwargs = {"alpha": 1}
        if output == None and model == None:
            output = self.model_usage.forward(input).question_answering_score
            model = self.model_usage.model  #error that this model doesn't have a method relprop
        ...
        #expects relevance propagation mechanism
        model.relprop(torch.tensor(one_hot_vector).to(output.device), **kwargs)

...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, mapper=None, wiki_emb=None, tokenizer=None, surface2wiki = None):
        ...
        debug = True  ## <--- SEE TO TRUE FOR DEV PURPOSES
        print("in Bert Embeddings with surface2wiki", surface2wiki)
       
        if debug and wiki_emb is not None:
            print('1', 'Yitzhak Ben - Zvi' in wiki_emb)  #False
            print('2', 'Yitzhak Ben-Zvi' in wiki_emb)    #True
            print('3', 'Yitzhak_Ben-Zvi' in wiki_emb)    #True

        if mapper is not None: 
            #slash_tok_id = tokenizer.encode("/", add_special_tokens=False)  #AttributeError: 'BertTokenizer' object has no attribute 'encode'
            slash_tok_id = torch.tensor(tokenizer.convert_tokens_to_ids("/"), dtype=torch.long).cuda()
            ...
            if debug:
                ...
                print("Mapper",mapper)
                print("WikiEmb",wiki_emb)
                print("tokenizer",tokenizer)
                print("surface2wiki?",surface2wiki == None)
                """
                InputIDS torch.Size([32, 100]) tensor([[ 101, 2040, 2003,  ...,    0,    0,    0], [ 101, 2079, 2035,  ...,    0,    0,    0], [ 101, 2040, 2003,  ...,    0,    0,    0],
                        ..., [ 101, 2020, 2035,  ...,    0,    0,    0], [ 101, 2001, 1996,  ...,    0,    0,    0], [ 101, 2129, 2116,  ...,    0,    0,    0]], device='cuda:0')
                Mapper <lxrt.entry.LinearMapper object at 0x7f0a7bb73a10>
                WikiEmb <lxrt.entry.Wikipedia2VecEmbedding object at 0x7f0a7bb7a210>
                tokenizer <lxrt.tokenization.BertTokenize
                """
            for r in range(input_ids.shape[0]):
                # find UNK token 100 in tensors to get list of ents
                cur_ex = input_ids[r]
                cur_ex_toks = tokenizer.convert_ids_to_tokens([int(t) for t in cur_ex])
                #cur_ex_str = tokenizer.convert_tokens_to_string(cur_ex_toks)    #BertTokenizer doesn't have this function
                try:
                ...
                for i in range(int(len(unk_locs) / 2)):
                   ...
                   if surface2wiki == None or ( surface2wiki[r] == [] ):
                       ...
                       try:
                           ...
                           print("Couldn't tokenize", ent_title_toks, "from start/end indices",start_ent_ind+1, end_ent_ind, "in cur_ex_tokens",cur_ex_toks)
                           continue
                   ...
                   try:
                       ent_vectors = np.array(wiki_emb[ent_title])         #hmmm... Exception: ('Embedding does not contain', 'Yitzhak Ben - Zvi')  <-- HERE  
                       if debug:
                ...
                for i in range(int(len(unk_locs) / 2)):
                    if i == 0:
                        ...
                        # for non-first ents
                        mid_ids = cur_ex[unk_locs[i-1]:unk_locs[i]]
                        mid_emb = self.word_embeddings(torch.tensor(mid_ids, dtype=torch.long).cuda())
                        if debug:
                            print("MID IDS:",i, mid_ids, len(mid_ids), tokenizer.convert_ids_to_tokens([int(m) for m in mid_ids]))
                            print("MID Emb",i, mid_emb.shape, mid_emb[0][0:5])
                        cur_embedding.append(mid_emb)  #get word embeddings for everything prior to prior end UNK
                    ...
                    if len(cur_ents) > i:
                        if len(cur_ents[i]) >= 4:
                            if cur_ents[i][3] != []:
                                ebert_emb = torch.tensor(cur_ents[i][3], dtype=torch.long).unsqueeze(0).cuda()  #unsqueeze to make it 1 x 768 so it matches dims of rest of cur_embeddings
                                # CURRENTLY HERE !   it seems unsqueeze(0) isn't make it 2d like'd hoped.. it zeros out things.. try in notebook and then fix here!
                                cur_embedding.append(ebert_emb)
                                cur_embedding.append(slash_emb)
                                if debug:
                                    print("EBERT ENT",i, ent_title, type(ebert_emb), type(slash_emb), cur_ex[(cur_ents[i][0]+1):(cur_ents[i][1]-1)]) 
                    ...
                    #add BERT based entity rep without UNKs
                    bert_ent_ids = cur_ex[(unk_locs[i]+1):(unk_locs[i+1])]
                    bert_ent_emb = self.word_embeddings(torch.tensor(bert_ent_ids, dtype=torch.long).cuda())
                    # NOTICE thrown by above line:  To copy construct from a tensor, 
                    # it is recommended to use sourceTensor.clone().detach() or sourceTensor.clone().detach().requires_grad_(True), 
                    # rather than torch.tensor(sourceTensor).

                    if debug:
                        print("BERT ENT IDS:",i, bert_ent_ids, len(bert_ent_ids), tokenizer.convert_ids_to_tokens([int(b) for b in bert_ent_ids]))
                        print("BERT ENT Emb",i, bert_ent_emb.shape, [ b[0:5] for b in bert_ent_emb ])
            ...
            # RuntimeError: stack expects each tensor to be equal size, but got [105, 768] at entry 0 and [119, 768] at entry 1
            # pad according to max len over 1st dimension
            #word_embeddings = torch.stack(word_embeddings).cuda()
            if debug:
                print("Input_ids", input_ids.shape)
                print("Word Embeddings sizes: ")
                for ind, w in enumerate(word_embeddings):   #all ending vals are 0s
                    #print(ind, w.shape, w[-10:][0:10])    #this shows first 10 values of last 10 embeddings for current sent
                    print(ind, w.shape, w[0:10][0:10])    #this shows first 10 values of first 10 embeddings for current sent

            #instead of expanding dimension of input sequence ( which all end in zeros ), instead remove all dimension past 100
            words_embeddings = torch.stack([ w[:input_ids.size(1)] for w in word_embeddings])
            # RuntimeError: stack expects each tensor to be equal size, but got [100, 768] at entry 0 and [32, 100, 768] at entry 8

            if debug:
                print("Final Word Embeddings:", words_embeddings.shape)
                print("Input_ids ", input_ids.shape)

            # verify its of the correct dimension (compared with non-EBERT version )
            # words_embeddings = words_embeddings.permute(1,0,2)
        ...
        if debug:
            print("Dims Words", words_embeddings.shape, "Positions: ", position_embeddings.shape, "TokenTypes: ", token_type_embeddings.shape)
...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config)  #BERT EMBEDDER TO CHANGE FOR EBERT
        self.encoder = LXRTEncoder(config)
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None, mapper = None, 
                wiki_emb = None, tokenizer = None, surface2wiki = None):

        print("in LXRTModel with surface2wiki", surface2wiki)
        if mapper is not None:
            token_type_ids = None
            embedding_output = self.embeddings(input_ids, token_type_ids, mapper, wiki_emb, tokenizer, surface2wiki)
        print("DONE with Embeddings Forward")
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, mode='lxr', mapper=None, wiki_emb=None, tokenizer=None):
        """
        ...
        """
        super().__init__(config)
        self.bert = LXRTModel(config)   # THIS IS THE BERT MODEL TO CHANGE
        self.mode = mode
        self.apply(self.init_bert_weights)
        self.mapper = mapper
        ...
        print("Setup LXRTFeature with mapper/wiki_emb", mapper, wiki_emb)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None, surface2wiki=None):
        # surface2wiki is to allow for mapping to Wiki ents shows surface form isn't the same as their Wiki link name
        #print("IN LXRTF with surface2wiki", surface2wiki)
        feat_seq, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                            visual_feats=visual_feats,
                                            visual_attention_mask=visual_attention_mask,
                                            mapper=self.mapper, wiki_emb=self.wiki_emb, 
                                            tokenizer=self.tokenizer, surface2wiki = surface2wiki)
        if 'x' == self.mode:
            return pooled_output

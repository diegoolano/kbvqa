...
if __name__ == "__main__":
    # SAMPLE CALL FROM README.md
    """
    python3 run_mapping.py 
	--src wikipedia2vec-base-cased 
	--tgt bert-base-cased 
	--save_out ../mappers/wikipedia2vec-base-cased.bert-base-cased.linear
    """
    args = parse_args()

    outputstring = f"{args.src}:{args.tgt}:{args.mapper}"       #mapper is in linear/mlp/ortho
    if args.do_em:
        outputstring += f"-em-{args.em_mapper}"
    
    train_dico = load_dico(args.dico)                    #by default loads: resources/bert-base-cased.all.dico.txt
    if args.do_test:
        if args.test_dico:
    ...
    tgt = load_embedding(args.tgt)             #see "code/embeddings.py"  which contains all code for Transformer and WikipediaVec embeddings
    ...
    # THERE are what do the mapper.train that is called in train_mapper.. 
    # see code/mappers.py for how to load .npy files 
    # these are the target embeddings that have been mapped to by minimizing the squared l2 norm from the source wikivec embeddings using words found in args.dico above ( 22K ents )

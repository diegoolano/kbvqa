...
def load_embedding(name, **kwargs): 
    if name.startswith("bert-") or name.startswith("xlnet-") or name.startswith("t5-") \
            or name.startswith("roberta-") or name.startswith("xlm-") or name.startswith("lxmert"):
...
class Wikipedia2VecEmbedding(Embedding):
    def __init__(self, path, prefix = "ENTITY/", do_cache_dict = True, do_lower_case = False):
        ...
        if os.path.exists(path):
            ...
            print("Loading ", os.path.join(RESOURCE_DIR, "wikipedia2vec", path))
            self.model = Wikipedia2Vec.load(os.path.join(RESOURCE_DIR, "wikipedia2vec", path))
        elif os.path.exists(os.path.join(DATA_RESOURCE_DIR, "wikipedia2vec", path)):
...
class TransformerEmbedding(Embedding):
    ...
    def get_model_and_tokenizer_class(name):
        ...
        if modeltype == "bert" or "pretraining" in name:
            ...
            if modeltype == "lxmert":
                ...
                lxrt_encoder = LXRTEncoder( [], max_seq_length=50)
                # --loadLXMERT   snap/pretrained/model
                #lxrt_encoder.load(args.load_lxmert)    ## CAN WE USE JUST THIS?  if not use load_qa
                lxrt_encoder.load("model")    ## copy snap/pretrained/model_LXRT.pth  here
                return lxrt_encoder.model.bert, lxrt_encoder.tokenizer
            else:
    ...
    def __init__(self, name):
        ...
        if name == "lxmert":
            ...
            print("loaded lxmert model/tokenizer", type(model), type(self.tokenizer) )
        else:
